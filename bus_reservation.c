...
#include <string.h> 
#define max 50
// Defining a structure to store bus information 
struct Bus { 
	int busNumber; 
	char source[max]; 
...
// Defining a structure to store passenger information 
struct Passenger { 
	char name[max]; 
	int age;
	int seatNumber; 
...
// Defining a structure to store user login information 
struct User { 
	char username[max];  // user1 
	char password[max];   // password1
}; 
...
{ 
	for (int i = 0; i < numUsers; i++) { 
		if (strcmp(users[i].username, username) == 0   // basic idea behind this user name is to identufy the number of user and same to keep the password simple
...
{ 
	...
	struct Passenger 
		passengers[500]; // Array to store passenger information 
